...
class EmployeeServiceTest {
    private final EmployeeDetailsProvider employeeDetailsProvider = Mockito.mock(EmployeeDetailsProvider.class);
    private final EmployeeService employeeService = new EmployeeService(employeeDetailsProvider);

    @Test
    void shouldInvokeEmployeeDetailsService(){
        employeeService.getEmployees();
        Mockito.verify(employeeDetailsProvider, Mockito.timeout(1)).getAllEmployees();
    }

    @Test
    void shouldReturnExpectedResponseAfterInvokingEmployeeDetailsService(){
        Mockito.when(employeeDetailsProvider.getAllEmployees()).thenReturn(new Employees(List.of(new Employee("1", "Peter", 27, 1899877))));
    ...
    void shouldReturnAllEmployeesIfSearchTokenMatchesName(){
        Mockito.when(employeeDetailsProvider.getAllEmployees()).thenReturn(new Employees(
    ...
    void shouldReturnEmployeeIfThereExistEmployeeWithId(){
        Mockito.when(employeeDetailsProvider.getEmployeeById("1")).thenReturn(Optional.of(new Employee("2", "John", 27, 289987)));

        Optional<Employee> employeesById = employeeService.getEmployeesById("1");

        Mockito.verify(employeeDetailsProvider, Mockito.times(1)).getEmployeeById("1");
    ...
    void shouldReturnOptionalOfEmptyIfThereExistNoEmployeeWithId(){
        Mockito.when(employeeDetailsProvider.getEmployeeById("1")).thenReturn(Optional.empty());
    ...
    void shouldGetSuccessStatusIfEmployeeCreatedSuccessFully(){
        Mockito.when(employeeDetailsProvider.createEmployee(any())).thenReturn("Success");
    ...
    void shouldDeleteEmployeeSuccessfully() {
        Mockito.when(employeeDetailsProvider.deleteEmployee("1")).thenReturn("Success");
